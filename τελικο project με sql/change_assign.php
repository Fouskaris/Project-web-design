...
$stud_info = isset($_POST["student"]) ? trim($_POST["student"]) : "";
...
if ($stud_info) {
    foreach ($students as &$student) {
        ...
        if ($student['student_number'] == $stud_info || $fullName == $stud_info) {
            $student_number = $student['student_number'];
            $message = "Ο κύριος $prof_surname σας ανέθεσε το θέμα: $subj_name";
            $new_id = uniqid();
            $newNotifStud = [
                'id' => $new_id,
                'message' => $message,
...
foreach ($subjects as &$subject) {
    if ($subject['id'] == $subj_id) {
        ...
        if ($student_number) {
            $subject['student_number'] = $student_number;
        }
...
echo '<script>alert("Η ανάθεση και οι αλλαγές καταχωρήθηκαν με επιτυχία!"); history.back();</script>';
exit;
?>
