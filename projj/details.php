...
$Subjid = isset($_POST['ia']) ? (int)$_POST['ia'] : null;
$jsonString = file_get_contents("dipl.json");
$data = json_decode($jsonString, true);

if ($Subjid !== null && isset($data['subjects']) && is_array($data['subjects'])) {
  $subjectFound = null;
  ...
  if ($subjectFound) {
    ...
    echo '<a href="subjList.php">
    <button class="button" type="button">Επιστροφή</button>
    </a>';
    echo '</div>';
    echo '</div>';
  } else {
    echo "<div class='container'><p>Δεν βρέθηκε θέμα με ID: <strong>$Subjid</strong>.</p></div>";
  }
} else {
